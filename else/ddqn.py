...
for i in range(5000):
    ...
    while True:
        ...
        r = (env.theta_threshold_radians - abs(_s[2])) / env.theta_threshold_radians * 7  \
            + (env.x_threshold - abs(_s[0])) / env.x_threshold * 3
        # 经验回放
        ...
        if store_count > store_size:
            ...
            # action = net(b_s_).detach().max(1)[1].reshape(b_size, 1)
            action = torch.argmax(net(b_s_), dim=1).reshape(b_size, 1)
            q_next = net2(b_s_).detach().gather(1, action.long())
